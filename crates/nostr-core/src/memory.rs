...
impl NostrMemory {
    ...
    /// Add a owner note to an npub's memory.
    pub async fn add_npub_owner_note(&self, hex_pubkey: &str, note: &str) {
        let mut store = self.store.write().await;
        if let Some(npub) = store.npubs.get_mut(hex_pubkey) {
            npub.owner_notes.push(note.to_string());
            drop(store);
            *self.dirty.write().await = true;
        }
    }
    ...
    pub async fn build_context(&self, sender_hex: &str, group_id: &str) -> String {
        ...
        if let Some(npub) = store.npubs.get(sender_hex) {
            ...
            if !npub.owner_notes.is_empty() {
                parts.push(format!("owner says: {}", npub.owner_notes.join("; ")));
            }
